...
; Jaan Soulier
; 101189819
; To find my modifications, search for "Jaan Soulier"
...
(define (eval-1 exp)
  (cond ((constant? exp) exp)
    ...
    ; Jaan Soulier: I added a case for and that forwards to and-1.
    ((and-exp? exp) (and-1 (cdr exp)))

    ((symbol? exp) (eval exp))  ; use underlying Racket's EVAL

    ; Jaan Soulier: I added a check to ensure multiple arguments were not passed.
    ((quote-exp? exp)
      (if (> (length exp) 2) (error "bad expr: " exp)
      (cadr exp)))
    ...
    ; Jaan Soulier: I added the eval for exp.
    ((lambda-exp? exp) (eval exp))

    ; Jaan Soulier: I added a case for map-1 that forwards to eval-1 and apply-1.
    ((map-exp? exp) (eval-1 (cons map (cdr exp))))

    ((pair? exp) (apply-1 (eval-1 (car exp))      ; eval the operator
                  (map eval-1 (cdr exp))))
    (else (error "bad expr: " exp))))

; Jaan Soulier: I created and-1 for evaluating and.
(define (and-1 exp)
  (define (recurse items previous)
    (if (empty? items) previous
      (let ((curr (eval-1 (car items))))
      (if (not curr) #f
      (recurse (cdr items) curr)))))
  (recurse exp #t))
...
; Jaan Soulier: I added the map-exp? procedure. 
(define map-exp? (exp-checker 'map-1))

; Jaan Soulier: I added the and-exp? procedure. 
